...
const streaksHook = process.env.STREAKS_HOOK;
...
function isRequestAllowed(ip) {
	...
	if (currentTime - cacheEntry.lastRequest < timeWindowMs && cacheEntry.count >= maxRequests) {
...
class UserManager {
	...
	getUserByName(name) {
		this.loadUsers();
		let result = this.users.find((u) => u.name && u.name.toLowerCase() === name.toLowerCase());
		if (result) {
			return {
				success: true,
				user: result,
	...
	getUserByTrimmedName(name) {
		...
		let result = this.users.find(
			(u) => u.name && removeMiddle(u.name.toLowerCase()) === removeMiddle(name.toLowerCase()),
		);
		if (result) {
			return {
				success: true,
	...
	writeUser(user) {
		...
		if (this.users && user) {
			...
			if (userIndex !== -1) {
				...
				fs.writeFile('./data/users.json', JSON.stringify(this.users, null, 4), (err) => {
					if (err) {
...
app.post('/shop/purchase', (req, res) => {
	...
	if (!userManager.getUserByRequest(req).user) {
		...
		if (requestUser.Shekels < item.price && requestUser.id != process.env.DEBUG_ID) {
			return res.status(403).send({
				success: false,
				message:
					'You need ' + (item.price - requestUser.Shekels) + ' more Shekels to purchase this item.',
			});
		}
...
app.get('/streaks/bump', async (req, res) => {
	...
	if (userIndex !== -1) {
		...
		if (requestUser.visits) {
			if (Date.now() - requestUser.visits[requestUser.visits.length - 1] < 518400000) {
				return res.status(403).send({
					success: false,
		...
		sendWebhook(
			streaksHook,
			'Streak Bump',
			`${requestUser.name} bumped their streak to ${streak + 1}.\n\nThey now have ${
				requestUser.Shekels
			} Shekels.`,
			7855479,
			'',
				...
				.then((userResponse) => {
					const guildResponse = axios
						...
						.then((guildResponse) => {
							...
							const userIndex = userArray.findIndex((u) => u.discordID === user.id);
							let foundUser = null;
							if (userIndex !== -1) {
							...
							res.redirect(`https://stogacs.club/leaderboard?access_token=${access_token}`);
						})
						.catch((error) => {
...
app.post('/me/logout', (req, res) => {
	...
	if (!userObj.user) {
		...
		const all = req.query.hasOwnProperty('all') && req.query.all.toLowerCase() === 'true';
		if (all) {
			user.tokens = [];
...
app.post('/leaderboard/claim', async (req, res) => {
	...
	const inServer = currentUserGuilds.some((guild) => guild.id === '1009284359334924349');
	if (!inServer)
		return res.status(403).json({
			success: false,
			message: 'You must be in the StogaCS Discord server to claim your account.',
		});
	...
	if (badNames.some((badName) => body.name.toLowerCase().includes(badName)))
		return res.status(403).json({
			success: false,
			message: 'Your name may need admin approval. Please, contact a club officer.',
		});
...
app.post('/me/login', (req, res) => {
	...
	if (timeUntilRequestOk != 0) {
		return res.status(429).send({
			success: false,
			message: "You've sent too may requests to this endpoint. Please try again later.",
...
console.log(
	'Running server with version identifier ' + sign(ver, process.env.SIGNING_KEY).split('.')[1],
);
...
function sendWebhook(URL, title, description, color, footer, avatar) {
	let webhookData = {
		embeds: [
			{
				title: title,
	...
	const sendWebhookRequest = async () => {
		...
		try {
			response = await axios.post(URL, webhookData);
